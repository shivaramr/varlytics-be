...
def run_garch_n_simulation(symbol: str, num_simulations: int = 10000, num_days: int = 252, include_chart_data: bool = False):
    """
    GARCH(1,1) with Normal distribution simulation.
    ...
    # P(min) = probability that final price is within 5% of minimum
    p_min = float(np.sum(final_prices <= min_price * 1.05) / num_simulations)
    
    # P(max) = probability that final price is within 5% of maximum
