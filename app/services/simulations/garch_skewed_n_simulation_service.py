...
def run_garch_skewed_n_simulation(symbol: str, num_simulations: int = 10000, num_days: int = 252, include_chart_data: bool = False):
    """
    GARCH(1,1) with Skewed Normal distribution simulation.
    ...
    # Fit GARCH(1,1) with Skewed Normal distribution
    model = arch_model(log_returns, vol='GARCH', p=1, q=1, dist='skewt')
    ...
    # Monte Carlo simulation (using normal with skew adjustment)
    dt = 1 / 252
    last_price = hist['Close'].iloc[-1]
    
    shocks = np.random.normal(0, 1, size=(num_days, num_simulations))
    # Apply slight negative skew for downside emphasis
    shocks = shocks - 0.1 * np.abs(shocks)
