...
def run_gjr_garch_skewed_t_simulation(symbol: str, num_simulations: int = 10000, num_days: int = 252, include_chart_data: bool = False):
    """
    GJR-GARCH(1,1) with Skewed Student's t distribution simulation.
    ...
    # Fit GJR-GARCH(1,1,1) with Skewed t distribution
    model = arch_model(log_returns, vol='GARCH', p=1, o=1, q=1, dist='skewt')
    ...
    # Monte Carlo simulation with skewed distribution
    dt = 1 / 252
    last_price = hist['Close'].iloc[-1]
    
    shocks = skewnorm.rvs(-2, size=(num_days, num_simulations))
